...
import type { WordsQuery } from "./words.schema.js";

export const wordsRepository = {
  async list(userId: string, q: WordsQuery) {
    const wordWhere: Prisma.WordWhereInput = {};
    if (q.category) wordWhere.category = q.category;
    if (q.search) {
      wordWhere.OR = [
  ...
  random(excludeId: number, count: number) {
    ...
    >`SELECT id, word, bangla, definition, part_of_speech
       FROM words
       WHERE id <> ${excludeId}
       ORDER BY random()
       LIMIT ${count}`;
